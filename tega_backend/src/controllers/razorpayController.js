    ...
    if (!offer) {
      ...
      offer = allOffers.find(
        (o) =>
          ...
          student.institute
            .toLowerCase()
            .includes(o.instituteName.toLowerCase())
      );
    }
...
export const createOrder = async (req, res) => {
  try {
    ...
    if (packageId) {
      try {
        ...
        if (!offer) {
          ...
          if (mongoose.Types.ObjectId.isValid(packageId)) {
            offer = await Offer.findOne({
              "packageOffers._id": new mongoose.Types.ObjectId(packageId),
            });
          }
        }
        ...
        if (!offer) {
          ...
          if (!packageOffer && offer.packageOffers.length > 0) {
            ...
            packageOffer = offer.packageOffers.find(
        ...
        const packageInstitute = (packageOffer.instituteName || "")
          .toLowerCase()
          .trim();

        if (
          studentInstitute !== packageInstitute &&
          !studentInstitute.includes(packageInstitute) &&
          !packageInstitute.includes(studentInstitute)
        ) {
        ...
        const PackageTransaction = (
          await import("../models/PackageTransaction.js")
        ).default;
      ...
      const existingPayment = await RazorpayPayment.checkExistingPayment(
        studentId,
        null,
        examId
      );
    ...
    if (isPackage) {
      ...
      if (offerInfo && offerInfo.hasOffer) {
        ...
        try {
          ...
          let offer = await Offer.findOne({
            ...
            $or: [
              {
                "tegaExamOffers.examId": examId,
                "tegaExamOffers.isActive": true,
              },
              {
                "tegaExamOffer.examId": examId,
          ...
          if (!offer) {
            ...
            const partialMatch = allOffers.find(
              (o) =>
                o.instituteName
                  .toLowerCase()
                  .includes(student.institute.toLowerCase()) ||
          ...
          if (offer) {
            ...
            // Check for old structure (tegaExamOffer single object)
              ...
              offer.tegaExamOffer &&
              offer.tegaExamOffer.examId.toString() === examId &&
              offer.tegaExamOffer.isActive
            ) {
      ...
      const preferredAmount =
        offerInfo &&
        offerInfo.hasOffer &&
        !Number.isNaN(rawFrontendFinalPrice) &&
        rawFrontendFinalPrice > 0
      ...
      if (
        offerInfo &&
        offerInfo.hasOffer &&
        (rawFrontendFinalPrice > 0 || rawFrontendOfferPrice > 0)
      ) {
        offerDetails = {
    ...
    const orderOptions = {
      ...
      currency: "INR",
      receipt: isPackage
        ? `TEGA_PACKAGE_${Date.now().toString().slice(-8)}_${studentId
            .toString()
            .slice(-8)}`
        : isTegaExam
        ? `TEGA_EXAM_${Date.now().toString().slice(-8)}_${studentId
            .toString()
            .slice(-8)}`
        : `TEGA_COURSE_${Date.now().toString().slice(-8)}_${studentId
            .toString()
            .slice(-8)}`,
      notes: {
    ...
    try {
      ...
      return res.status(500).json({
        ...
        stack:
          process.env.NODE_ENV === "development" ? orderError.stack : undefined,
      });
    }
    ...
    const payment = new RazorpayPayment({
      ...
      description: isPackage
        ...
        : isTegaExam
        ? `Payment for TEGA exam: ${courseName}${
            offerDetails
              ? ` (Institute Offer: ${offerDetails.discountPercentage}% off)`
              : ""
          }`
      ...
      examId: isPackage
        ? packageData?.includedExam?.examId || null
        : examId || null,
      attemptNumber: attemptNumber || null,
      isRetake: isRetake || false,
      isTegaExam: isTegaExam,
...
export const verifyPayment = async (req, res) => {
  try {
    ...
    if (payment.isPackage && payment.packageId && payment.packageData) {
      const { completePackagePurchase } = await import(
      ...
      if (!transaction) {
        try {
          ...
          let offer = await Offer.findOne({
            "packageOffers._id": payment.packageId,
          });
          let packageOffer = null;
          ...
          if (offer) {
            ...
            if (mongoose.Types.ObjectId.isValid(payment.packageId)) {
              offer = await Offer.findOne({
                "packageOffers._id": new mongoose.Types.ObjectId(
                  payment.packageId
                ),
              });
              if (offer) {
            ...
            if (!packageOffer) {
              ...
              for (const o of allOffers) {
                const pkg = o.packageOffers.find(
                  (p) =>
                    p._id.toString() === payment.packageId ||
                    p._id.equals?.(payment.packageId)
                );
                if (pkg) {
                  offer = o;
                  packageOffer = pkg;
