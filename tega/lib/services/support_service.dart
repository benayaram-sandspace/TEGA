...
class SupportService {
  ...
  Future<void> loadData() async {
    ...
    try {
      final String jsonString = await rootBundle.loadString('lib/data/support_data.json');
  ...
  Future<bool> addMessageToTicket(String ticketId, SupportMessage message) async {
    await loadData();
    final index = _tickets.indexWhere((ticket) => ticket.id == ticketId);
    if (index != -1) {
      final updatedMessages = List<SupportMessage>.from(_tickets[index].messages);
