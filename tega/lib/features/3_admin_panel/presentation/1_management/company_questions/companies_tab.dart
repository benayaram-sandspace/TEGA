...
class _CompaniesTabState extends State<CompaniesTab> {
  ...
  Future<void> _loadFromCache() async {
    try {
      ...
      final cachedCompanies = await _cacheService
          .getCompanyListWithDetailsData();
  ...
  Future<void> _loadCompanies({bool forceRefresh = false}) async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        throw Exception(
          errorData['message'] ??
              'Failed to fetch companies: ${res.statusCode}',
        );
      }
    } catch (e) {
  ...
  Widget _buildCompanyCard(
    ...
    final companyName = (company['name'] ?? company['companyName'] ?? 'Unknown')
        .toString();
