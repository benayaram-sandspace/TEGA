...
class ProgressReportGeneratorPage extends StatefulWidget {
  ...
  @override
  State<ProgressReportGeneratorPage> createState() =>
      _ProgressReportGeneratorPageState();
}

class _ProgressReportGeneratorPageState
    extends State<ProgressReportGeneratorPage> {
  ...
  final college_service.CollegeService _collegeService =
  ...
  void initState() {
    ...
    _reportNameController.text =
        '${widget.student.name}_Progress_Report_${DateTime.now().millisecondsSinceEpoch}';
    _loadDataFromJson();
  }
        ...
        for (var jsonStudent in college.students) {
          ...
          _allStudents.add(
            Student.detailed(
              ...
              jobReadiness:
                  (jsonStudent.skillScore / 100.0), // Convert to 0-1 scale
            ),
          );
        }
      }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            ..._reportSections.keys.map(
              (section) => _buildCheckboxItemWithData(section),
            ),
            const SizedBox(height: 24),
      ...
      child: Column(
        ...
        children: [
          if (section == 'Student Profile Summary') ...[
            ...
            _buildDropdownEditor('Timeframe', 'timeframe', data, [
              '3 months',
              '6 months',
              '1 year',
            ]),
          ...
          build: (pw.Context context) {
            return [
              ...
              if (widget.student.studentId != null)
                pw.Text('Student ID: ${widget.student.studentId}'),
              if (widget.student.branch != null)
                pw.Text('Course: ${widget.student.branch}'),
              if (widget.student.yearOfStudy != null)
                pw.Text('Year: ${widget.student.yearOfStudy}'),
              pw.SizedBox(height: 20),

              // Selected Sections
              pw.Header(level: 1, child: pw.Text('Report Sections')),
              ...selectedSections.map((section) => pw.Text('• $section')),
              pw.SizedBox(height: 20),

              // Section Details
              for (String section in selectedSections) ...[
                pw.Header(level: 2, child: pw.Text(section)),
      ...
      case 'Skill Graph Analysis':
        return pw.Column(
          ...
          children: [
            ...
            pw.Text(
              'Include Recommendations: ${data['includeRecommendations']}',
            ),
          ],
        );
    ...
    try {
      ...
      if (result.type == ResultType.done) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'File opened successfully: ${file.path.split('/').last}',
            ),
