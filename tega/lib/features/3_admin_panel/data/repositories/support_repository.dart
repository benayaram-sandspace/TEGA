...
class SupportRepository {
  ...
  Future<List<SupportTicket>> getTicketsByPriority(
  ...
  Future<List<SupportTicket>> getTicketsByCategory(
    ...
    try {
      ...
      if (index != -1) {
        ...
        await _logTicketEvent(
          ...
          description:
              'Status changed from ${oldTicket.status.name} to ${status.name}',
  ...
  Future<Map<String, dynamic>> getTicketAnalytics({
    ...
    final inProgressTickets = filteredTickets
        ...
        .length;
    final resolvedTickets = filteredTickets
    ...
    final overdueTickets = filteredTickets
        .where(
          (t) =>
              t.slaDeadline != null &&
    ...
    return {
      ...
      'slaCompliance': totalTickets > 0
          ? ((totalTickets - overdueTickets) / totalTickets) * 100
          : 0,
    };
  }
  ...
  Future<Map<String, dynamic>> getAgentPerformance(String agentId) async {
    ...
    final agentTickets = _tickets
        .where((t) => t.assignedTo == agentId)
        .toList();
    final completedTickets = agentTickets
        .where(
          (t) =>
              t.status == TicketStatus.resolved ||
              t.status == TicketStatus.closed,
    ...
    final avgResolutionTime = completedTickets.isNotEmpty
        ? completedTickets
                  .map((t) => t.updatedAt?.difference(t.createdAt).inHours ?? 0)
  ...
  // Bulk Operations
  Future<bool> bulkUpdateTicketStatus(
  ...
  Future<List<TicketEvent>> getTicketHistory(String ticketId) async {
    await loadData();
    return _ticketEvents
        .where((event) => event.description.contains(ticketId))
        .toList();
  }

  // SLA Management
  Future<List<SupportTicket>> getTicketsNearSLA({
  ...
  Future<bool> updateSatisfactionRating(
    String ticketId,
    int rating,
    String comments,
  ) async {
