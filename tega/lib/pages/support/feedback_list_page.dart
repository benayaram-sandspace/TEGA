    ...
    setState(() {
      switch (index) {
        case 0: // All
          ...
          break;
        case 1: // To Do
          _filteredFeedback = _allFeedback.where((f) => f.status == 'new').toList();
          break;
        case 2: // In Progress
          _filteredFeedback = _allFeedback.where((f) => f.status == 'in_progress').toList();
          break;
        case 3: // Done
          _filteredFeedback = _allFeedback.where((f) => f.status == 'completed').toList();
          break;
      }
    });
  }
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          if (feedback.status == 'new')
            ElevatedButton(
              onPressed: () async {
                await _supportService.updateFeedbackStatus(feedback.id, 'in_progress');
